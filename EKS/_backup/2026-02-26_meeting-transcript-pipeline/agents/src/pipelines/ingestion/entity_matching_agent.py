"""
Entity Matching Agent - Vincula├º├úo inteligente de termos extra├¡dos ao grafo Neo4j
Usa similaridade lexical (fuzzy matching) e thesaurus para encontrar correspond├¬ncias

Este agente ├® respons├ível por:
1. Receber termos/entidades extra├¡dos pelo LLM
2. Buscar correspond├¬ncias no Neo4j usando m├║ltiplas estrat├®gias
3. Retornar sugest├Áes de vincula├º├úo com scores de confian├ºa
"""
...
class EntityMatchingAgent:
    """
    Agente especializado em vincular termos extra├¡dos ao grafo Neo4j.
    
    Estrat├®gias de matching (em ordem de prioridade):
    1. Match exato no nome can├┤nico
    2. Match exato em aliases (thesaurus)
    3. Match fuzzy usando dist├óncia de Levenshtein
    4. Match parcial (cont├®m)
    
    Configura├º├Áes:
    - fuzzy_threshold: Score m├¡nimo para considerar match fuzzy (default: 0.7)
    - partial_threshold: Score m├¡nimo para match parcial (default: 0.6)
    """
    ...
    def normalize(self, text: str) -> str:
        """Normaliza texto para compara├º├úo (lowercase, sem acentos, sem pontua├º├úo)"""
        if not text:
            return ""
        # Lowercase
        ...
        # Remove pontua├º├úo e espa├ºos extras
        text = re.sub(r'[^\w\s]', '', text)
        text = re.sub(r'\s+', ' ', text)
        return text
    
    def levenshtein_distance(self, s1: str, s2: str) -> int:
        """Calcula dist├óncia de Levenshtein entre duas strings"""
        if len(s1) < len(s2):
    ...
    def partial_match_score(self, term: str, target: str) -> float:
        """
        Score para match parcial (um cont├®m o outro).
        ...
        # Match de substring: se term est├í contido em target (ex: "montreal" em "montreal ventures")
        if n_term in n_target:
            # Boost se o termo come├ºa no in├¡cio do target (ex: "montreal" em "montreal ventures")
            if n_target.startswith(n_term):
                base_score = len(n_term) / len(n_target)
                # Boost adicional para termos que s├úo palavras completas no in├¡cio
        ...
        # Match reverso: se target est├í contido em term
        if n_target in n_term:
            return min(0.7, len(n_target) / len(n_term))
        
        # Word-based matching: verifica se todas as palavras do term est├úo no target
        ...
        if term_words and all(word in target_words for word in term_words):
            # Todas as palavras do term est├úo no target
    ...
    async def match_entity(self, term: str) -> MatchResult:
        ...
        Returns:
            MatchResult com candidatos e sugest├úo de a├º├úo
        """
        # Carrega nodes se ainda n├úo carregou
        ...
        if not candidates:
            return MatchResult(
                ...
                candidates=[],
                best_match=None,
                suggested_action="create"  # Sugerir cria├º├úo de novo node
            )
        
        best = candidates[0]
        
        # Determinar a├º├úo sugerida baseado no score
        if best.score >= 0.9:
            action = "link"  # Alta confian├ºa - pode linkar automaticamente
        elif best.score >= 0.7:
            action = "review"  # M├®dia confian├ºa - curador deve validar
        else:
            action = "create"  # Baixa confian├ºa - provavelmente ├® novo
    ...
    async def match_entities(self, terms: List[str]) -> List[MatchResult]:
        """Processa m├║ltiplos termos em batch"""
        if not self._loaded and self.neo4j_driver:
            await self.load_graph_nodes()
