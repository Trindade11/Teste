"""
Ingestion Router - Endpoints para pipeline de ingest├úo
Exp├Áe EntityMatchingAgent e outros agentes de ingest├úo
"""
...
class MatchEntitiesRequest(BaseModel):
    """Request para match de m├║ltiplas entidades"""
...
class MatchEntitiesResponse(BaseModel):
    """Response do match de m├║ltiplas entidades"""
    success: bool
    ...
    if _entity_matching_agent is None:
        ...
        if neo4j_client.driver:
            try:
                ...
                logger.info(f"Ô£à EntityMatchingAgent loaded {len(_entity_matching_agent.graph_nodes)} nodes")
            except Exception as e:
                logger.warning(f"ÔÜá´©Å Failed to load graph nodes: {e}")
...
async def match_entity(
    ...
    """
    Busca correspond├¬ncia de um termo no grafo Neo4j.
    ...
    Returns:
        MatchResult com candidatos e sugest├úo de a├º├úo (link/review/create)
    """
    try:
...
async def match_entities(
    ...
    """
    Busca correspond├¬ncia de m├║ltiplos termos no grafo Neo4j (batch).
...
async def get_graph_nodes(
    ...
    """
    Lista os nodes carregados do grafo (para debug/verifica├º├úo).
...
async def reload_graph_nodes(
    ...
    ├Ütil quando novos nodes s├úo adicionados e o cache precisa ser atualizado.
    """
    try:
