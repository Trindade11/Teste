# Template de Post LinkedIn - EKS

## Metadados
- **Título de trabalho**: Onboarding em 8 Fases - Do Perfil ao Agente Profissional
- **Pilar**: [ ] Técnico [x] Organizacional [ ] Conceitual [ ] Tendência [ ] Case
- **Data planejada**: 27/01/2026 08:30 (cadência: seg/qui 08:30)
- **Status**: [x] Rascunho [ ] Revisão [ ] Pronto [ ] Publicado
- **Imagem**: Screenshot da tela final do OnboardingWizard ("Seu Agente Profissional foi Criado")

---

## Hook (primeiras 2-3 linhas)

> Em menos de 5 minutos, cada colaborador ganha um assistente de IA que 𝗰𝗼𝗻𝗵𝗲𝗰𝗲 𝘀𝗲𝘂 𝗰𝗼𝗻𝘁𝗲𝘅𝘁𝗼.

**Tipo de hook usado**:
- [ ] Provocação direta
- [ ] Metáfora forte
- [ ] Experiência pessoal
- [x] Benefício direto
- [ ] Pergunta

---

## Corpo do Post

### Problema/Contexto

A maioria dos assistentes de IA corporativos trata todo mundo igual.

Não importa se você é do financeiro ou do RH.
Se é gestor ou operacional.
Se prefere respostas diretas ou detalhadas.

Resultado: respostas genéricas.
Respostas que não servem para ninguém de verdade.

### Contraste

Agora imagine um onboarding guiado.
5 minutos. 8 etapas. Zero fricção.

E no final: um agente que sabe quem você é, onde você está na organização, e o que você precisa.

### Conceito-chave

Eu chamo isso de 𝗣𝗲𝗿𝘀𝗼𝗻𝗮 𝗞𝗻𝗼𝘄𝗹𝗲𝗱𝗴𝗲 𝗣𝗿𝗼𝗳𝗶𝗹𝗲 (𝗣𝗞𝗣).

Não é um formulário de cadastro.
É a construção progressiva de um perfil que a IA usa para te entender.

### Lista/Estrutura

As 8 fases do onboarding:

1️⃣ 𝗕𝗲𝗺-𝘃𝗶𝗻𝗱𝗼 - Contexto do sistema
2️⃣ 𝗣𝗲𝗿𝗳𝗶𝗹 - Nome, cargo, sobre você
3️⃣ 𝗢𝗿𝗴𝗮𝗻𝗶𝘇𝗮çã𝗼 - Empresa, departamento, sua função
4️⃣ 𝗢𝗿𝗴𝗮𝗻𝗼𝗴𝗿𝗮𝗺𝗮 - Validar posição (gestor, pares, subordinados)
5️⃣ 𝗖𝗼𝗺𝗽𝗲𝘁ê𝗻𝗰𝗶𝗮𝘀 - Habilidades técnicas e comportamentais
6️⃣ 𝗢𝗯𝗷𝗲𝘁𝗶𝘃𝗼𝘀 - O que busca, quais desafios enfrenta
7️⃣ 𝗥𝗲𝘃𝗶𝘀ã𝗼 - Confirmar antes de criar
8️⃣ 𝗔𝗴𝗲𝗻𝘁𝗲 𝗖𝗿𝗶𝗮𝗱𝗼 - Pronto para usar

### Benefício/Consequência

O que isso muda na prática:

• O agente sabe seu papel na organização
• Sabe com quem você trabalha (gestor, pares)
• Conhece suas competências
• Entende seus objetivos e desafios

A partir daí, cada resposta é contextualizada.
Não é mais "IA genérica".
É 𝘀𝗲𝘂 𝗮𝗴𝗲𝗻𝘁𝗲 𝗽𝗿𝗼𝗳𝗶𝘀𝘀𝗶𝗼𝗻𝗮𝗹.

Na imagem mostro a tela final: o agente já criado, com seu nome, cargo, competências e diagrama de como ele opera.

E o melhor: esse perfil evolui com o tempo.
A IA aprende com cada interação.
E propõe atualizações que você valida.

---

## CTA (Call-to-Action)

**Tipo**:
- [x] Pergunta para comentários
- [ ] Link para recurso
- [ ] Pedido de compartilhamento
- [x] Convite para grupo/comunidade

> Quanto tempo leva para um novo colaborador entender como a empresa funciona?
> E se a IA já soubesse isso desde o primeiro dia?
>
> Se fizer sentido, eu deixo o link do grupo no WhatsApp no 1º comentário.

---

## Comentário 1 (fixado) - copiar e colar

```
🔗 Eu e quase 100 profissionais de ponta conversamos sobre EKS, ontologia corporativa e IA corporativa (grupo):
https://chat.whatsapp.com/Fyp7W6h2PXS99z5l1MX5Kc
 
Se você trabalha com processos, gestão do conhecimento ou arquitetura, entra lá — eu compartilho bastidores e padrões práticos.
```

## Elementos Visuais

- [ ] Imagem de grafo/diagrama
- [x] Screenshot de interface (OnboardingWizard - tela final)
- [ ] Carrossel
- [ ] Sem imagem

**Descrição da imagem**: Screenshot da tela "Seu Agente Profissional foi Criado" mostrando:
- Avatar com iniciais do usuário
- Nome do agente (Agente [Nome])
- Cargo e empresa
- Objetivo principal
- Competências-chave
- Diagrama Mermaid de como o agente funciona
- Próximos passos

---

## Menções e Hashtags

### Pessoas/Empresas para mencionar
- (nenhuma específica neste post)

### Hashtags (regra: não usar hashtags, apenas termos no texto)
- (sem hashtags - termos já incorporados no texto naturalmente)

---

## Checklist Pré-Publicação

- [x] Hook forte nos primeiros 150 caracteres
- [x] Pelo menos 1 termo próprio (PKP, Agente Profissional)
- [x] Conexão técnico ↔ negócio clara
- [x] Frases curtas intercaladas com longas
- [x] Destaque Unicode para conceitos-chave
- [x] Parágrafos curtos (máx 3 linhas)
- [x] CTA claro
- [x] Hashtags relevantes (regra: não usar hashtags)
- [x] Imagem preparada (screenshot do OnboardingWizard - tela "Seu Agente Profissional foi Criado")
- [ ] Revisão de erros de digitação
- [ ] Melhor horário: 8h-9h ou 17h-18h

---

## Texto Final (copiar e colar)

```
Em menos de 5 minutos, cada colaborador ganha um assistente de IA que 𝗰𝗼𝗻𝗵𝗲𝗰𝗲 𝘀𝗲𝘂 𝗰𝗼𝗻𝘁𝗲𝘅𝘁𝗼.

A maioria dos assistentes de IA corporativos trata todo mundo igual.

Não importa se você é do financeiro ou do RH.
Se é gestor ou operacional.
Se prefere respostas diretas ou detalhadas.

Resultado: respostas genéricas.
Respostas que não servem para ninguém de verdade.

Agora imagine um onboarding guiado.
5 minutos. 8 etapas. Zero fricção.

E no final: um agente que sabe quem você é, onde você está na organização, e o que você precisa.

Eu chamo isso de 𝗣𝗲𝗿𝘀𝗼𝗻𝗮 𝗞𝗻𝗼𝘄𝗹𝗲𝗱𝗴𝗲 𝗣𝗿𝗼𝗳𝗶𝗹𝗲 (𝗣𝗞𝗣).

As 8 fases do onboarding:

1️⃣ 𝗕𝗲𝗺-𝘃𝗶𝗻𝗱𝗼 - Contexto do sistema
2️⃣ 𝗣𝗲𝗿𝗳𝗶𝗹 - Nome, cargo, sobre você
3️⃣ 𝗢𝗿𝗴𝗮𝗻𝗶𝘇𝗮çã𝗼 - Empresa, departamento, sua função
4️⃣ 𝗢𝗿𝗴𝗮𝗻𝗼𝗴𝗿𝗮𝗺𝗮 - Validar posição (gestor, pares, subordinados)
5️⃣ 𝗖𝗼𝗺𝗽𝗲𝘁ê𝗻𝗰𝗶𝗮𝘀 - Habilidades técnicas e comportamentais
6️⃣ 𝗢𝗯𝗷𝗲𝘁𝗶𝘃𝗼𝘀 - O que busca, quais desafios enfrenta
7️⃣ 𝗥𝗲𝘃𝗶𝘀ã𝗼 - Confirmar antes de criar
8️⃣ 𝗔𝗴𝗲𝗻𝘁𝗲 𝗖𝗿𝗶𝗮𝗱𝗼 - Pronto para usar

O que isso muda na prática:

• O agente sabe seu papel na organização
• Sabe com quem você trabalha (gestor, pares)
• Conhece suas competências
• Entende seus objetivos e desafios

A partir daí, cada resposta é contextualizada.
Não é mais "IA genérica".
É 𝘀𝗲𝘂 𝗮𝗴𝗲𝗻𝘁𝗲 𝗽𝗿𝗼𝗳𝗶𝘀𝘀𝗶𝗼𝗻𝗮𝗹.

Na imagem mostro a tela final: o agente já criado, com seu nome, cargo, competências e diagrama de como ele opera.

E o melhor: esse perfil evolui com o tempo.
A IA aprende com cada interação.
E propõe atualizações que você valida.

Quanto tempo leva para um novo colaborador entender como a empresa funciona?
E se a IA já soubesse isso desde o primeiro dia?

Se fizer sentido, eu deixo o link do grupo no WhatsApp no 1º comentário.
```

---

## Pós-Publicação

- **URL do post**: 
- **Horário publicado**: 
- **Métricas 24h**: Impressões: | Reações: | Comentários:
- **Métricas 7d**: Impressões: | Reações: | Comentários:
- **Aprendizados**: 

---

*Rascunho v1.0 - 19/01/2026*

